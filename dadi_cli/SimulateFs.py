...
def simulate_demography(model, model_file, p0, ns, pts_l, misid, output, inference_file):
    if pts_l == None:
...
def simulate_demes(demes_file, ns, pts_l, pop_ids, output):
    if pts_l == None:
        pts_l = (int(ns*1.1)+2, int(ns*1.2)+4, int(ns*1.3)+6)
    fs = dadi.Spectrum.from_demes(demes_file, sampled_demes=pop_ids, sample_sizes=ns, pts=pts_l)
    fs.to_file(output)
