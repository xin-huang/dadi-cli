...
duplicated_sele_models = ['IM', 'IM_pre', 'IM_pre_single_gamma', 'IM_single_gamma', 
                          'split_asym_mig', 'split_asym_mig_single_gamma', 'split_mig', 
                          'three_epoch', 'two_epoch', 'split_mig_single_gamma']
...
for m in duplicated_models: oned_models.remove(m)
for m in duplicated_models: twod_models.remove(m)
for m in duplicated_sele_models: sele_models.remove(m)
...
def get_model(model_name, model_file=None):
    ...
    else: raise ValueError(f'Cannot find model: {model_name}.')

    try:
        params = func.__param_names__
    except:
        raise ValueError(f'Demographic model needs a .__param_names__ attribute!\nAdd one by adding the line ' + model_name0 +
...
def print_built_in_models():
    ...
    for m in oned_models: print(f'- {m}')
    print()

    print('Built-in 2D demographic models:')
    for m in twod_models: print(f'- {m}')
    print()

    print('Built-in demographic models with selection:')
    for m in sele_models: print(f'- {m}')
...
def print_built_in_model_details(model_name):
    ...
    try: print(f'- {model_name}:\n\n\t{model_doc_new}\n')
    except: raise ValueError(f'Cannot find model: {model_name}.')
