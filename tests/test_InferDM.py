...
def test_infer_demography_code():
    ...
    infer_demography(fs, func, p0, pts_l, upper_bounds, lower_bounds, 
                     fixed_params, misid, cuda, maxeval, maxtime)
...
def test_infer_global_opt_code():
    ...
    ll_global, popt, theta = infer_global_opt(fs, func, p0, pts_l, upper_bounds, lower_bounds, 
                     fixed_params, misid, cuda, maxeval, maxtime, global_algorithm, seed=12345)
...
def test_infer_demography_bash(files):
    ...
    fits = glob.glob("./tests/test_results/example.two_epoch.demo.params.InferDM.opts.*")
...
def test_InferDM_seed(files):
    optimizations = 3
    subprocess.run(
        pytest.bash_command.replace('.demo.','.demo.seeded.') + str(optimizations) + " --threads " + str(optimizations) + " --seed 12345" , shell=True
    )
    subprocess.run(
        pytest.bash_command.replace('.demo.','.demo.seeded.') + str(optimizations) + " --threads " + str(optimizations) + " --seed 12345" , shell=True
    )
    fits_list = glob.glob("./tests/test_results/simulation.two_epoch.demo.seeded.params.InferDM.opts.*")
    fits_list.sort()
...
def test_InferDM_wq(files):
    ...
    print(
        pytest.bash_command.replace('.demo.','.demo.workqueue.') + str(optimizations) + ' --debug-wq ' +
    ...
    # WorkQueue 
    time.sleep(20)
    fits = glob.glob("./tests/test_results/example.two_epoch.demo.workqueue.params.InferDM.opts.*")
    number_of_fits = sum([ele.startswith('#') != True for ele in open(fits[-1]).readlines()])
    assert optimizations == number_of_fits
...
def test_InferDM_seed(files):
    ...
    fits_list = glob.glob("./tests/test_results/example.two_epoch.demo.seeded.params.InferDM.opts.*")
    fits_list.sort()
    fits1 = [line.strip() for line in open(fits_list[-2],'r').readlines()]
    fits1.sort()
    fits2 = [line.strip() for line in open(fits_list[-1],'r').readlines()]
...
def test_InferDM_wq_seed(files):
    optimizations = 3
    factory = subprocess.Popen(
        "work_queue_factory -T local -M test-dm-seed-two-epoch -P ./tests/mypwfile --workers-per-cycle=0 -t 10 --cores=1  -w " + str(optimizations), 
        shell=True
        )
    subprocess.run(
        pytest.bash_command.replace('.demo.','.demo.workqueue.seed.') + str(optimizations) + " --seed 12345 " +
        "--work-queue test-dm-seed-two-epoch ./tests/mypwfile", shell=True
        )
    subprocess.run(
        pytest.bash_command.replace('.demo.','.demo.workqueue.seed.') + str(optimizations) + " --seed 12345 " +
        "--work-queue test-dm-seed-two-epoch ./tests/mypwfile", shell=True
    ...
    factory.kill()
    time.sleep(20)
    fits_list = glob.glob("./tests/test_results/simulation.two_epoch.demo.workqueue.seed.params.InferDM.opts.*")
    fits_list.sort()
    fits1 = [line.strip() for line in open(fits_list[-2],'r').readlines()]
    fits1.sort()
    fits2 = [line.strip() for line in open(fits_list[-1],'r').readlines()]
