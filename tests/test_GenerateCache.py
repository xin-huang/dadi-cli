...
def test_generate_cache_code(capsys):
    generate_cache(func=DFE.DemogSelModels.two_epoch_sel, grids=[120, 140, 160], popt="./tests/example_data/example.two_epoch.demo.params.InferDM.bestfits", 
                   gamma_bounds=[1e-4, 2000], gamma_pts=50, output="./tests/test_results/cache_large_two_epoch_code.bpkl", sample_sizes=[20],
                   additional_gammas=[], mp=False, cuda=False, dimensionality=1)
...
def test_generate_cache_custom_model_code(capsys):
    sel_func, params = get_model('split_mig_fix_T_sel', 'tests/example_data/example_models.py')
    generate_cache(func=sel_func, grids=[120, 140, 160], popt="./tests/example_data/example.split_mig_fix_T.demo.params.InferDM.bestfits", 
                   gamma_bounds=[1e-4, 20], gamma_pts=5, output="./tests/test_results/cache_small_2s_split_mig_fix_T_sel_code.bpkl", sample_sizes=[20, 20],
                   additional_gammas=[], mp=False, cuda=False, dimensionality=2)

    assert os.path.exists("./tests/test_results/cache_small_2s_split_mig_fix_T_sel_code.bpkl")
    s = pickle.load(open('./tests/test_results/cache_small_2s_split_mig_fix_T_sel_code.bpkl','rb'))
