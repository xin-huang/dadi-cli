...
def test_generate_fs(data):
    # Unfolded
    generate_fs(vcf=pytest.vcf, output=pytest.unfolded_output, pop_ids=['YRI', 'CEU'], pop_info=pytest.pop_info, 
    ...
    # Folded
    generate_fs(vcf=pytest.vcf, output=pytest.folded_output, pop_ids=['YRI', 'CEU'], pop_info=pytest.pop_info, 
                projections=[216, 198], marginalize_pops=None, subsample=False, polarized=False, 
                bootstrap=None, chunk_size=None, masking='', seed=None)
    ...
    # Projection
    generate_fs(vcf=pytest.vcf, output=pytest.unfolded_output, pop_ids=['YRI', 'CEU'], pop_info=pytest.pop_info, 
                projections=[30, 20], marginalize_pops=None, subsample=False, polarized=True, 
                bootstrap=None, chunk_size=None, masking='', seed=None)
    dadi_cli_fs = dadi.Spectrum.from_file(pytest.unfolded_output)
...
def test_generate_fs_masks(data):
    ...
    generate_fs(vcf=pytest.vcf, output=pytest.shared_singleton_mask_output, pop_ids=['pop0', 'pop1', 'pop2'], pop_info=pytest.three_pop_info, 
                projections=[98, 216, 100], marginalize_pops=None, subsample=False, polarized=True, bootstrap=None, chunk_size=None, 
                masking='singletons', seed=None)
    dadi_cli_fs = dadi.Spectrum.from_file(pytest.shared_singleton_mask_output)
    ...
    with pytest.raises(ValueError) as e_info:
        generate_fs(vcf=pytest.vcf, output=pytest.shared_singleton_mask_output, pop_ids=['pop0', 'pop1', 'pop2', 'pop3'], pop_info=pytest.four_pop_info, 
                    projections=[98, 196, 100, 20], marginalize_pops=None, subsample=False, polarized=True, bootstrap=None, chunk_size=None, 
                    masking='shared', seed=None)

    assert str(e_info.value) == "Masking singletons is only supported for a frequency spectrum with no more than 3 populations."
