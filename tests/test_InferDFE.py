...
def test_infer_dfe_code():
    ...
    infer_dfe(fs, cache1d, cache2d, sele_dist, sele_dist2, theta,
              p0, upper_bounds, lower_bounds, fixed_params, misid, cuda, maxeval, maxtime)
...
def test_InferDFE_bash(capsys):
    ...
    subprocess.run(
        ...
        "--output-prefix ./tests/test_results/example.two_epoch.dfe.params --optimizations " + str(optimizations), shell=True
    )
    fits = glob.glob("./tests/test_results/example.two_epoch.dfe.params.InferDFE.opts.*")
...
def test_InferDFE_wq(capsys):
    ...
    subprocess.run(
        ...
        "--demo-popt " + fits_fid + " --pdf1d lognormal --ratio 2.31 " +
        "--p0 1 1 --ubounds 10 10 --lbounds 10e-3 10e-3 --nomisid " + 
        "--output-prefix ./tests/test_results/example.two_epoch.dfe.workqueue.params --optimizations " + str(optimizations) + ' ' +
    ...
    factory.kill()
    time.sleep(20)
    fits = glob.glob("./tests/test_results/example.two_epoch.dfe.workqueue.params.InferDFE.opts.*")
    number_of_fits = sum([ele.startswith('#') != True for ele in open(fits[-1]).readlines()])
    assert optimizations == number_of_fits
...
def test_InferDFE_seed(capsys):
    ...
    fits_list = glob.glob("./tests/test_results/example.two_epoch.dfe.seeded.params.InferDFE.opts.*")
    fits_list.sort()
...
def test_InferDFE_wq(capsys):
    ...
    fits_fid = "./tests/example_data/example.two_epoch.demo.params.InferDM.bestfits"
    factory = subprocess.Popen(
        "work_queue_factory -T local -M test-dfe-seed-two-epoch -P ./tests/mypwfile -t 10 --workers-per-cycle=0 --cores=1  -w " + str(optimizations), 
        shell=True#, preexec_fn=os.setsid
        )
    subprocess.run(
        "dadi-cli InferDFE " + 
        ...
        "--demo-popt " + fits_fid + " --pdf1d lognormal --ratio 2.31 " +
        "--p0 1 1 --ubounds 10 10 --lbounds 10e-3 10e-3 --nomisid " + 
        "--output-prefix ./tests/test_results/simulation.two_epoch.dfe.workqueue.seed.params --optimizations " + str(optimizations) + " --seed 12345 " +
        "--work-queue test-dfe-seed-two-epoch ./tests/mypwfile", shell=True
    )
    subprocess.run(
        "dadi-cli InferDFE " + 
        "--fs ./tests/example_data/two_epoch_non.fs --cache1d ./tests/example_data/cache_two_epoch_1d.bpkl " +
    ...
    factory.kill()
    time.sleep(20)
    fits_list = glob.glob("./tests/test_results/simulation.two_epoch.dfe.workqueue.seed.params.InferDFE.opts.*")
    fits_list.sort()
    fits1 = [line.strip() for line in open(fits_list[-2],'r').readlines()]
    fits1.sort()
    fits2 = [line.strip() for line in open(fits_list[-1],'r').readlines()]
