...
def test_GenerateFs(capsys):
    subprocess.run(["dadi-cli", "GenerateFs", 
        "--vcf", "./example_data/1KG.YRI.CEU.biallelic.synonymous.snps.withanc.strict.short.vcf", 
        "--pop-info", "../examples/data/1KG.YRI.CEU.popfile.txt", 
        "--pop-ids", "YRI","CEU", "--projections", "216","198", "--polarized", 
        "--output", "./test_results/1KG.YRI.CEU.synonymous.snps.unfold.short.fs"
    ])
    dadi.cli_fs = dadi.Spectrum.from_file('test_results/1KG.YRI.CEU.synonymous.snps.unfold.short.fs')

    datafile = './example_data/1KG.YRI.CEU.biallelic.synonymous.snps.withanc.strict.short.vcf'
...
def test_BestFit_DM(capsys):
    ...
    assert os.path.exists("./test_results/simulation.two_epoch.demo.params.InferDM.bestfits")
...
def test_GenerateCache(capsys):
    ...
    assert os.path.exists("./test_results/cache_large_two_epoch.bpkl")
...
def test_BestFit_DFE(capsys):
    subprocess.run(
        "dadi-cli BestFit --input-prefix ./test_results/simulation.two_epoch.dfe.params.InferDFE " +
        "--pdf lognormal --lbounds 10e-3 10e-3 --ubounds 10 10", shell=True
    )
    assert os.path.exists("./test_results/simulation.two_epoch.dfe.params.InferDFE.bestfits")
